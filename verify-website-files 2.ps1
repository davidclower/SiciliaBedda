...
# Check for translations-data.js (referenced in HTML files)
Write-Host ""
Write-Host "Checking referenced translation files..." -ForegroundColor Yellow
$translationsFile = Join-Path $projectRoot "translations-data.js"
if (-not (Test-Path $translationsFile)) {
    $warnings += "MISSING: translations-data.js (referenced in HTML files)"
    Write-Host "  [WARNING] Missing: translations-data.js" -ForegroundColor Yellow
} else {
    Write-Host "  [OK] translations-data.js" -ForegroundColor Green
}
