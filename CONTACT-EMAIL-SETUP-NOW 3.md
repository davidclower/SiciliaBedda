...
4. Add these **3 environment variables** (for **Production** environment):

   | Variable Name | Value |
   |---------------|-------|
   | `EMAIL_SERVICE` | `gmail` |
   | `GMAIL_USER` | `info@siciliabedda.com` |
   | `GMAIL_APP_PASSWORD` | *Paste your 16-character App Password from Step 1* (spaces optional) |

   Optional: `CONTACT_EMAIL` (defaults to info@siciliabedda.com if not set).
...
- Check that `EMAIL_SERVICE=gmail`, `GMAIL_USER`, and `GMAIL_APP_PASSWORD` are set in Vercel
- Verify `GMAIL_APP_PASSWORD` is the App Password (not your normal Gmail password)
...
**Still not working?**
- Double-check that `EMAIL_SERVICE` is set to `gmail` (not `log` or `smtp`)
